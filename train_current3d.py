...
CONFIG = {
    "model_name": "3D_padding_128_allimages",
...
from monai.transforms import (
    ...
    NormalizeIntensityd, ResizeWithPadOrCropd, Orientationd
)

def get_padding_cropping_transforms(spatial_size):
    return Compose([
...
train_ds = MonaiDataset(data=train_files, transform=get_padding_cropping_transforms(CONFIG['roi_size']))
train_loader = DataLoader(train_ds, batch_size=CONFIG["batch_size"], shuffle=True)

test_files = [{"image": p, "label": l} for p, l in zip(test_df['path'], test_df['label'])]
test_ds = MonaiDataset(data=test_files, transform=get_padding_cropping_transforms(CONFIG['roi_size']))
