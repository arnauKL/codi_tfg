MODEL_NAME = "model_2_2D_baseline"
...
# Function to sum intensities along the axial axis (usually the last dim)
# You can change dim=1, 2, or 3 depending on which orientation you want to collapse
def sum_slices(data):
    # data is a tensor of shape (C, H, W, D)
    # We sum across the last dimension (Depth/Axial)
    return torch.sum(data, dim=-1)

# Example: Only sum slices 30 to 45 where the striatum usually lives
def sum_striatum_only(data):
    # this is just an idea, Im not even calling this function for now
    return torch.sum(data[:, :, :, 30:45], dim=-1)

data_transforms = Compose([
    LoadImaged(keys=["image"]),
    EnsureChannelFirstd(keys=["image"]),
    CenterSpatialCropd(keys=["image"], roi_size=(76, 76, 76)),
    # Sum intensities to turn (1, 76, 76, 76) into (1, 76, 76)
    Lambdad(keys=["image"], func=sum_slices), 
    NormalizeIntensityd(keys=["image"]), # make intensities stay btween 0 n 1
])
...
for epoch in range(num_epochs):
    print(f"\nEpoch {epoch}/{num_epochs} {100*(epoch+1)/num_epochs}% done", end="")
