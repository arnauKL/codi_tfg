...
rawdata = '/home/data/PPMI/rawdata/'

df = pd.read_csv(rawdata + 'participants.tsv', sep='\t')
label_map = dict(zip(df['participant_id'], df['cohort'])) # mapeja IDs al cohort

# passa de cohorts a binari (1: PD, 0: HC, els altres s'ignoren)
def get_binary_label(cohort_code) -> int:
	if cohort_code == 1: # PD
		return 1
	if cohort_code == 2: # HC
		return 0
...
baseline_images = glob.glob(f"{rawdata}sub-*/ses-BL/spect/*_DaTSCAN.nii.gz") # agafam totes les imatges
data_list = []

for img_path in baseline_images:

    sub_id = img_path.split('/')[-4] # només 'sub-PPMI100001'	
    patid = sub_id.replace('sub-', '')
	
    label = get_binary_label(label_map.get(patid))
    
    if label is not None:
		# Path to the baseline scan
        img_path = os.path.join(rawdata, sub_id, 'ses-BL', 'spect', f'{sub_id}_ses-BL_DaTSCAN.nii.gz')
...
clean_df.to_csv("ppmi_rawdata_sesBL_mapping.csv", index=False)
#print(clean_df.info())

from sklearn.model_selection import train_test_split

df = pd.read_csv("ppmi_rawdata_sesBL_mapping.csv")
...
for epoch in range(num_epochs):
    print(f"\nEpoch {epoch+1}/{num_epochs} {100*(epoch)/num_epochs}% done", end="")
