...
data_transforms = Compose([
    ...
    CenterSpatialCropd(keys=["image"], roi_size=(new_size, new_size, new_size)),
...
optimizer = optim.Adam(model.parameters(), lr=0.0001) # learning rate was at 0.0001

####### save stuff into a csv so I can see later

import csv

log_file = "training_log_unbalanced_v2.csv"
...
for epoch in range(num_epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), "checkpoints/best_model_unbalanced.pth")
        print("Experimental 'best' model saved")

    # save a just in case
    torch.save(model.state_dict(), "checkpoints/latest_model_unbalanced.pth")
    print("-" * 30)
